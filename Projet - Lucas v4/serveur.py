###----------LANCEMENT-DE-SQLITE--------------------------On est passé à PostGreSQL donc cette partie sert moins
import sqlite3
...
@app.get('/parametreAuto', response_class=HTMLResponse) 
def ParametreAuto(request:Request) -> str :
    return templates.TemplateResponse('parametreAuto.html',{'request': request,'title':'Projet'})

@app.get("/consulter", response_class=HTMLResponse)
def Consulter(request:Request) -> str:

    ###-----Requete-SQLITE-quelconque--------- On est passé à PostGreSQL donc cette partie est à modifier
    connection= create_connection("./test.db")  #Ouvre le SQLite database
    cur = connection.cursor()               #Execute un 'query'
    cur.execute("SELECT * FROM ARTICLE")    #Execute un 'query' (bis)
    results = cur.fetchall()    #Affiche le resultat de la requete
    ###---------------------------------------
    
    data = str(results[1][0])
...
# http://127.0.0.1:8000/ vous verrez notre site
### -------------------------------------------------------
